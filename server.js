 ...
 *
 * Routes:
 *   POST /api/auth/qf/exchange  — code + code_verifier → tokens + decoded user
 ...
 *   QF_REDIRECT_URI   — furkan://oauth/callback
 *   QF_USE_PRELIVE    — "true" for pre-live, omit for production
...
app.post("/api/auth/qf/exchange", async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...
      user, // { sub, email, name, first_name, ... }
    });
  } catch (err) {
